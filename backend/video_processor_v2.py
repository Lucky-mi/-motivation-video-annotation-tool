...
import json
import time
# Google Generative AI 导入（使用标准SDK）
try:
...
class SmartVideoProcessor:
    ...
    def analyze_video_with_gemini(self, video_path: str) -> Dict:
        ...
        print(f"✅ 上传完成，文件ID: {uploaded_file.name}。正在等待处理...")

        # --- 开始修复 ---
        # 增加一个等待循环，直到文件状态变为 ACTIVE
        # 注意：v1beta的SDK可能没有 .state.name，而是直接 .state
        # 为了保险起见，我们假设有 state.name
        current_state = ""
        try:
            current_state = uploaded_file.state.name
        except AttributeError:
            # 兼容旧版SDK
            file_details = genai.get_file(uploaded_file.name)
            current_state = file_details.state.name

        while current_state == "PROCESSING":
            print("⏳ 文件仍在处理中 (状态: PROCESSING)，等待10秒...")
            time.sleep(10)  # 等待10秒
            # 重新获取文件状态
            file_details = genai.get_file(uploaded_file.name)
            current_state = file_details.state.name
        ...
        if current_state != "ACTIVE":
            print(f"❌ 文件处理失败，最终状态: {current_state}")
            raise Exception(f"文件 {uploaded_file.name} 处理失败，状态为: {current_state}")

        print("✅ 文件处理完成，状态: ACTIVE")
        # --- 结束修复 ---
        ...
        try:
            ...
            print(f"✅ 分析完成，找到 {len(analysis.get('key_moments', []))} 个关键时刻")
            return analysis
