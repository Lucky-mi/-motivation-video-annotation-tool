...
def filter_already_processed(videos: list, downloader: VideoDownloader) -> list:
    """
    过滤已经处理过的视频
    检查：1. 数据库中的URL且文件存在（包括在ai_check_errors中的）
    """
    video_dir = Path("data/Youtube_videos")
    error_dir = Path("data/ai_check_errors")

    # 从数据库获取已处理的URL，并验证文件是否真实存在
    existing_urls = set()
    for video in downloader.links_db["videos"]:
        url = video["url"]
        ...
        if video_path:
            ...
            # 如果原路径不存在，检查是否在 ai_check_errors 文件夹中
            # （文件可能被移动但数据库路径未更新）
            ...
            # 也检查 Youtube_videos 文件夹（防止路径记录错误）
            video_path_in_dir = video_dir / path.name
            if video_path_in_dir.exists():
                existing_urls.add(url)
                continue

    # 统计 ai_check_errors 中的视频数量
    error_count = 0
    if error_dir.exists():
        error_count = len(list(error_dir.glob("*.mp4")))
...
def main():
    ...
    for batch_idx in range(total_batches):
        ...
        if content_filter:
            ...
            try:
                ...
                # 创建错误视频存储目录
                ERROR_VIDEO_DIR = Path("data/ai_check_errors")
                ERROR_VIDEO_DIR.mkdir(parents=True, exist_ok=True)
                ...
                for video_data in downloaded:
                    ...
                    if is_parsing_error or is_system_error:
                        ...
                        print(f"    ⚠️ {error_type}: {Path(video_path).name} - 标记为待审核")
                    else:
                        # 正常审核结果
                        approved = review_result.get('pass', False)

                    # ====== 处理视频文件 ======
                    final_path = video_path  # 默认路径不变
                    ...
                    if (is_parsing_error or is_system_error):
                        ...
                        try:
                            ...
                            if src_path.exists():
                                target_path = ERROR_VIDEO_DIR / src_path.name
                                shutil.move(str(src_path), str(target_path))
                                final_path = str(target_path)
                                print(f"       ↳ 已移至: {target_path.name}")
                        except Exception as move_err:
                            print(f"       ❌ 移动文件失败: {move_err}")
                    elif not approved and AUTO_DELETE_REJECTED:
                        # 审核未通过 且 开启自动删除 -> 删除文件
                        try:
                    ...
                    if approved is True:
                        ...
                        rejected_count += 1
                        batch_rejected += 1
                    # approved is None: pending_count已在上面累加

                print(f"  📊 本批: ✅ {batch_approved} | ❌ {batch_rejected} | ⏳ {batch_pending}")
                ...
                # 审核失败时，将视频标记为待审核（approved=None）
                print(f"  💾 将 {len(downloaded)} 个视频标记为待审核...")
                for video_data in downloaded:
                    video_info = video_data['info']
                    video_path = video_data['path']
                    downloader.add_video_link(
                        ...
                        duration=video_data['duration'],
                        keyword=video_info.get('search_keyword', 'Unknown'),
                        approved=None,  # 标记为待审核
                        review_reason=f"AI审核异常: {str(e)}",
    ...
    if content_filter:
        ...
        if pending_count > 0:
            print(f"⏳ 待审核: {pending_count} (AI解析异常)")
        if failed_count > 0:
